...
import { ENEMIES, AREAS, ITEMS, calcLevelUp, calcEquippedStats } from '../data/master.js';
import { getPlayer, updatePlayer } from '../database/db.js';
import { getPartyById } from './party.js';
...
export function processPartyTurn(partyId) {
  ...
  for (const [uid, action] of Object.entries(memberActions)) {
    ...
    if (action === 'attack') {
      const st = calcEquippedStats(player);
      const dmg = calcDamage(st.atk, enemy.def);
      totalDamageToEnemy += dmg;
      logs.push(`⚔️ **${player.name}**の攻撃： **${dmg}** ダメージ`);

    } else if (action === 'skill') {
      if (player.mp >= 10) {
        const stS = calcEquippedStats(player);
      const dmg = calcDamage(Math.floor(stS.atk * 1.6), enemy.def);
        totalDamageToEnemy += dmg;
  ...
  for (const uid of members) {
    ...
    // 回復していたメンバーにも攻撃が来る
    const stD = calcEquippedStats(player);
    const dmg = calcDamage(enemy.atk, stD.def);
