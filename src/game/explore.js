...
const RARE_ENEMIES = [
  { name: 'メタルスライム', hpRate: 0.6, atkRate: 1.2, defRate: 3, expRate: 3, goldRate: 3 },
  { name: '暴走ゴブリンキング', hpRate: 2.5, atkRate: 2, defRate: 1.5, expRate: 3, goldRate: 3 },
  { name: 'シャドウナイト', hpRate: 2, atkRate: 2.4, defRate: 1.8, expRate: 3, goldRate: 3 },
];

function pickWeighted(table) {
  const total = table.reduce((sum, event) => sum + event.weight, 0);
...
function updateExploreQuest(userId, areaKey) {
...
class ExploreEventHandler {
  ...
  pickEnemy() {
    ...
    return { enemyKey, enemy };
  }
}

class BattleEventHandler extends ExploreEventHandler {
  handle() {
    const { enemyKey, enemy } = this.pickEnemy();
    return {
      type: 'battle',
      title: 'エンカウント',
...
class TreasureEventHandler extends ExploreEventHandler {
  handle() {
    ...
    return {
      type: 'treasure',
      title: '宝箱発見',
      message: `**${this.area.name}** を探索中、宝箱を見つけた！\n**${gold}G** を手に入れた。`,
      gold,
    };
  }
}

class NpcEventHandler extends ExploreEventHandler {
  handle() {
    const npc = pickOne([
      { name: '旅の商人', message: 'この先は魔物が多い。回復薬の準備を忘れるなよ。', imageKey: 'traveling_merchant' },
      { name: '老婦人', message: '草原の風が強い日は、珍しいものが見つかると言われておるよ。', imageKey: 'old_woman' },
...
class ChoiceEventHandler extends ExploreEventHandler {
  handle() {
    return {
      type: 'choice',
      title: '怪しい祭壇',
...
class RareEnemyEventHandler extends ExploreEventHandler {
  handle() {
    const { enemyKey, enemy } = this.pickEnemy();
    const baseEnemy = ENEMIES[enemyKey];
    const rare = pickOne(RARE_ENEMIES);
    const hp = Math.max(1, Math.floor(baseEnemy.hp * rare.hpRate));
    Object.assign(enemy, {
      name: rare.name,
      hp,
      currentHp: hp,
...
class HiddenRoomEventHandler extends ExploreEventHandler {
  handle() {
...
class NothingEventHandler extends ExploreEventHandler {
  handle() {
...
class BossIntrusionEventHandler extends ExploreEventHandler {
  handle() {
    const { enemyKey, enemy } = this.pickEnemy();
    const baseEnemy = ENEMIES[enemyKey];
    const hp = Math.max(1, Math.floor(baseEnemy.hp * 2.2));
    Object.assign(enemy, {
...
export function explore(userId, areaKey) {
  ...
  updateExploreQuest(userId, areaKey);
